...
class Waveform(DPT.DPObject):
    ...
    def plot(self, i = None, ax = None, getNumEvents = False, getLevels = False,\
        ...
        if plot_type == 'Channel':  # plot in channel level
            ...
            self.plot_data(i, ax, plotOpts, False)                
        elif plot_type == 'Array':
            self.remove_subplots(fig)
            advals = np.array([*self.array_dict.values()])
            # set the starting index cstart for array i
            # set the ending index cend for array i
            # first array: 0 to advals[0] ; second array: advals[0]+1 to advals[1]
            # third array: advals[1]+1 to advals[2] ; last array: advals[2]+1 to advals[3]
            if i == 0:
                cstart = 0
                cend = advals[i]
            else:
    ...
    # create multiple subplots, and plot the data for the appropriate channel in each of the subplots
    # plot the data for one channel into a function that can be called when plotting one channel, 
    # or be called multiple times when plotting the channels that make up the array plot
    ...
    # remove existing subplots (for array/channel subplots)
